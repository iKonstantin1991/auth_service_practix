...
@pytest.mark.asyncio
async def test_get_roles_returns_roles_for_superuser(
    client: Client, superuser: TestUser
) -> None:
    access_token, _ = await login(superuser, client)
    response = await client.get(
        'api/v1/roles/', headers=build_headers(access_token)
    )
...
@pytest.mark.asyncio
async def test_get_roles_returns_forbidden_for_regular_user(
...
@pytest.mark.asyncio
async def test_get_roles_returns_unauthorized_if_no_token(
    client: Client,
) -> None:
...
@pytest.mark.asyncio
async def test_create_role_returns_role_for_superuser(
    client: Client, superuser: TestUser
) -> None:
...
@pytest.mark.asyncio
async def test_create_role_returns_forbidden_for_regular_user(
    client: Client, user: TestUser
) -> None:
    access_token, _ = await login(user, client)
    response = await client.post(
...
@pytest.mark.asyncio
async def test_create_role_returns_unauthorized_if_no_token(
    client: Client,
) -> None:
    response = await client.post(
        'api/v1/roles/',
...
async def test_delete_role_successfully_deletes_role_for_superuser(
    ...
    response = await client.delete(
        f'api/v1/roles/{role.id}', headers=build_headers(access_token)
    )
...
@pytest.mark.asyncio
async def test_delete_role_returns_forbidden_for_regular_user(
    client: Client, user: TestUser, role: Role
) -> None:
    access_token, _ = await login(user, client)
    response = await client.delete(
        f'api/v1/roles/{role.id}', headers=build_headers(access_token)
...
@pytest.mark.asyncio
async def test_delete_role_returns_unauthorized_if_no_token(
