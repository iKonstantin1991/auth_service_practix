...
@pytest.mark.asyncio
async def test_get_user_without_permission(client: Client, user: TestUser, superuser: TestUser) -> None:
    access_token, _ = await login(user, client)

    response = await client.get(f'api/v1/users/{superuser.id}', headers=build_headers(access_token))
...
@pytest.mark.asyncio
async def test_update_user_without_permission(client: Client, user: TestUser, superuser: TestUser) -> None:
    access_token, _ = await login(user, client)
    email_to_update = 'new_email@test.com'
    password_to_update = 'new_password'
...
async def test_get_user_roles_returns_roles_superuser(
    ...
    response = await client.get(f'api/v1/users/{user.id}/roles', headers=build_headers(access_token))
...
async def test_get_user_roles_returns_forbidden_regular_user(
    ...
    response = await client.get(f'api/v1/users/{superuser.id}/roles', headers=build_headers(access_token))
