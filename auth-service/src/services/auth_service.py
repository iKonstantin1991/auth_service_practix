...
class AccessTokenPayload(BaseTokenPayload):
    ...
    exp: float = Field(
        default_factory=lambda: time.time() + _ACCESS_TOKEN_EXPIRE_SECONDS
    )
    roles: List[str]
...
class RefreshTokenPayload(BaseTokenPayload):
    ...
    exp: float = Field(
        default_factory=lambda: time.time() + _REFRESH_TOKEN_EXPIRE_SECONDS
    )
    access_jti: UUID
...
class AuthService:
    ...
    async def create_token_pair(
        ...
        access_token_payload = AccessTokenPayload(
            user_id=user_id, roles=[r.name for r in roles]
        )
        refresh_token_payload = RefreshTokenPayload(
            user_id=user_id, access_jti=access_token_payload.jti
        )
        access_token = self._create_token(access_token_payload)
        refresh_token = self._create_token(refresh_token_payload)
        await self._token_storage.save_refresh_jti(
            refresh_token_payload.jti, _REFRESH_TOKEN_EXPIRE_SECONDS
        )
        return access_token, refresh_token

    def verify_password(
        self, user_email: str, plain_password: str, hashed_password: str
    ) -> bool:
        logger.info('Verifying password for user with email %s', user_email)
        return self._password_service.verify_password(
            user_email, plain_password, hashed_password
        )
    ...
    async def is_access_token_valid(self, access_token: str) -> bool:
        ...
        if payload.type != TokenType.ACCESS:
            ...
            return False
        return not await self._token_storage.check_access_token_revoked(
            payload.jti
        )

    async def check_is_valid_and_remove_refresh_token(
        ...
        if payload.type != TokenType.REFRESH:
            ...
            return False
        return await self._token_storage.check_refresh_token_exists(
            payload.jti
        )
    ...
    async def logout(self, refresh_token: str) -> None:
        payload = self._decode_refresh_token(refresh_token)
        logger.info(
            'Revoking refresh token with jti %s and access token with jti %s',
            payload.jti,
            payload.access_jti,
        )
        ...
        access_token_ttl = int(
            payload.iat + _ACCESS_TOKEN_EXPIRE_SECONDS - time.time()
        )
        if access_token_ttl > 0:
            await self._token_storage.save_revoked_access_jti(
                payload.access_jti, access_token_ttl
            )
    ...
    async def update_history(
        ...
        logger.info(
            'Updating auth history for user %s, user agent: %s',
            user_id,
            user_agent,
        )
    ...
    def _create_token(payload: BaseTokenPayload) -> str:
        return jwt.encode(
            payload.dict(), settings.private_key, algorithm=_ALGORITHM
        )

    @staticmethod
    def _decode_access_token(token: str) -> AccessTokenPayload:
        return AccessTokenPayload(
            **jwt.decode(token, settings.public_key, algorithms=[_ALGORITHM])
        )
