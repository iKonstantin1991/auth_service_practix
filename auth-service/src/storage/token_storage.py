...
class TokenStorage:
    ...
    async def save_refresh_jti(self, jti: UUID, ttl: int) -> None:
        ...
        try:
            await self.cache_storage.set(
                self._refresh_jti_cache_key(jti), _TOKEN_KEY, ttl
            )
    ...
    async def remove_refresh_jti(self, jti: UUID) -> None:
        ...
        try:
            ...
            logger.error(
                'Failed to remove refresh jti %s from cache: %s', jti, e
            )
            raise
    ...
    async def check_refresh_token_exists(self, jti: UUID) -> bool:
        ...
        try:
            jti = await self.cache_storage.getdel(
                self._refresh_jti_cache_key(jti)
            )
            return jti is not None
        except RedisError as e:
            logger.error(
                'Failed to delete refresh jti %s from cache: %s', jti, e
            )
            raise
    ...
    async def save_revoked_access_jti(self, jti: UUID, ttl: int) -> None:
        ...
        try:
            await self.cache_storage.set(
                self._revoked_access_jti_cache_key(jti), _TOKEN_KEY, ttl
            )
        except RedisError as e:
            logger.error(
                'Failed to save revoked access jti %s in cache: %s', jti, e
            )
            raise

    async def check_access_token_revoked(self, jti: UUID) -> bool:
        logger.info(
            'Checking if access token with jti %s was revoked in cache', jti
        )
        try:
            is_exist = await self.cache_storage.exists(
                self._revoked_access_jti_cache_key(jti)
            )
            return bool(is_exist)
        except RedisError as e:
            logger.error(
                'Failed to check if access token with jti %s is revoked in cache %s',
                jti,
                e,
            )
            raise
